# the plugin menu is part of the package uicilibris
#
...
def pMenu(d):
    result=" \\texttt{\\textbf{%s}\\nop}" %d[0] # \nop to avoid a sign processed by mediawiki's template engine
    result=result.replace("->",'$\\rightarrow$')
    return result

class menu(templateParser, plugin):
    def __init__(self):
